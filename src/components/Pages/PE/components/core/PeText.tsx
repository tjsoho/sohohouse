...
const PeText = ({ children, type, color, className }: Props) => {
  const renderHeadingText = () => {
    switch (type) {
      ...
      case "h3":
        return <h3 style={{ color, fontFamily: "Gopher" }}>{children}</h3>;
        case "sub1":
          return (
            <p style={{ color, fontFamily: "Gopher" }} className={`text-[20px] sm:text-[25px]  ${className}`}>
              {children}
            </p>
          );
    }
